...
export const identityBaseUrl = strip(
  import.meta.env.VITE_IDENTITY_API_BASE_URL,
  "https://identity.smt.tfnsolutions.us/api/v1",
);
export const identityStorageBase = identityBaseUrl.replace(/\/api\/v1\/?$/, "");
export const memoBaseUrl = strip(
  import.meta.env.VITE_MEMO_API_BASE_URL,
  "https://memo.smt.tfnsolutions.us/api/v1",
);
export const notificationBaseUrl = strip(
  import.meta.env.VITE_NOTIFICATION_API_BASE_URL,
  "https://notification.smt.tfnsolutions.us/api/v1",
);
export const settingsBaseUrl = strip(
  import.meta.env.VITE_SETTINGS_API_BASE_URL,
  "https://setting.smt.tfnsolutions.us/api/v1",
);
...
export const integrationAPI = {
  ...
  lookupUser: async (userId, token) => {
    const apiClient = createApiClient(token);
    const response = await apiClient.get(`/users/integration/lookup/${userId}`);
    return response.data;
  },
};
...
export const departmentAPI = {
  getDepartments: async (token, filters = {}) => {
    const apiClient = createApiClient(token);
    const params = {};
    if (filters.page) params.page = filters.page;
    if (filters.search) params.search = filters.search;
  ...
  updateDepartment: async (departmentId, departmentData, token) => {
    ...
    const response = await apiClient.put(`/departments/${departmentId}`, null, {
      params,
    });
    return response.data;
  },
...
export function buildWorkflowListParams(raw = {}) {
  ...
  if (raw.roles != null && raw.roles !== "") {
    const r = Array.isArray(raw.roles)
      ? raw.roles.filter(Boolean).join(",")
      : String(raw.roles);
    if (r) out.roles = r;
  }
  set(
    "estimated_time_min",
    raw.estimated_time_min != null && raw.estimated_time_min !== ""
      ? String(raw.estimated_time_min)
      : undefined,
  );
  set(
    "estimated_time_max",
    raw.estimated_time_max != null && raw.estimated_time_max !== ""
      ? String(raw.estimated_time_max)
      : undefined,
  );
...
export const workflowAPI = {
  ...
  toggleWorkflowStatus: async (workflowId, token) => {
    const apiClient = createMemoApiClient(token);
    const response = await apiClient.put(
      `/workflows/${workflowId}/toggle-status`,
    );
    return response.data;
  },
...
export const memoAPI = {
  createMemo: async (memoData, token) => {
    ...
    const hasAttachments =
      Array.isArray(memoData?.attachments) && memoData.attachments.length > 0;
    if (hasAttachments) {
      ...
      if (Array.isArray(memoData.recipients)) {
        memoData.recipients.forEach((rec, idx) => {
          if (rec?.recipient_id !== undefined) {
            fd.append(
              `recipients[${idx}][recipient_id]`,
              String(rec.recipient_id),
            );
          }
          if (rec?.recipient_type !== undefined) {
            fd.append(
              `recipients[${idx}][recipient_type]`,
              String(rec.recipient_type),
            );
          }
          if (rec?.recipient_role !== undefined) {
            fd.append(
              `recipients[${idx}][recipient_role]`,
              String(rec.recipient_role),
            );
          }
        });
      }
