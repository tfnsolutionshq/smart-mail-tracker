...
function CreateWorkflow() {
  ...
  const getRoleName = (roleIds) => {
    ...
    if (!roleIds) return "";
    const role = roles.find((r) => r.id === roleIds);
    ...
    try {
      ...
      if (isEditing && workflowResponse?.status && workflowResponse.data) {
        ...
        const steps = workflow.steps.map((step, index) => ({
          ...
          name: step.name,
          assignedRole: Array.isArray(step.required_role)
            ? step.required_role
            : step.required_role
              ? [step.required_role]
              : [],
  ...
  const handleSaveWorkflow = async () => {
    if (
      ...
      showNotification(
        "Please fill all required fields and add at least one step",
        "error",
      );
      return;
    }
    ...
    try {
      ...
      console.error(
        `Error ${isEditing ? "updating" : "creating"} workflow:`,
        error,
      );
      showNotification(
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-50">
          ...
          <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="hidden sm:block text-xs text-gray-600 truncate">
                Design your approval workflow with drag-and-drop steps
              </p>
            </div>
          </div>
      ...
      <div className="flex flex-1 min-h-0">
        ...
        {/* Sidebar */}
        <div
          className={`${sidebarOpen ? "w-80" : "w-0"} lg:${sidebarOpen ? "w-80" : "w-0"} bg-gray-50 border-r border-gray-200 transition-all duration-300 ease-in-out overflow-hidden lg:relative fixed inset-y-0 left-0 z-50`}
        >
              ...
              <div>
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="font-medium">
                      {workflowSteps.reduce(
                        (sum, step) => sum + (parseInt(step.timeLimit) || 48),
                        0,
                      )}
                      h
                    </span>
                  ...
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="font-medium">
                      {
                        workflowSteps.filter((step) => step.type === "approval")
                          .length
                      }
                    </span>
                  </div>
